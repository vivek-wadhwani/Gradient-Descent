...
public class GradientDescent {
	...
	// This is the main method where the all the functions of the various
	// classes are called.
		...
		if (s.isEmpty()) {
			...
			for (int j = 0; j < iterations - 1; j++) {
				...
				while (index < inputRecords.size()) {
					calculateValueOfA objA = new calculateValueOfA();
					a = objA.calcA(index, inputRecords, weights, size);
					y = calculateY(a);
					...
					c = uw.updateWeight(weights, gdash, inputRecords, index, y,
							eeta, c);
					index++;
				}
				index = 0;
			...
			System.out.println("Accuracy on training set("
					+ inputRecords.size() + " instances): " + df.format(c)
					+ "%");
			...
			for (int j = 0; j < iterations - 1; j++) {
				...
				while (index < input1Records.size()) {size
					calculateA caa1 = new calculateA();
					a = caa1.calcA(index, input1Records, weights, );
					y = calculateY(a);
					...
					c = uw1.updateWeight(weights, gdash, input1Records, index,
							y, eeta, c);
					index++;
				}
				index = 0;
			...
			System.out.println("Accuracy on test set(" + input1Records.size()
					+ " instances): " + df.format(c) + "%");
		}
	}

	// This method is used to initialize the weights of the neural network to
	// zero.
	...
	// This method is used to calculate the expected output of the particular
	// node in the neural network. In this case it is a sigmoid function.
